...
class NetModel:
	...
	def update_weights_adam(self,learning_rate=0.05, beta1 = 0.5, beta2 = 0.5, epsilon = 0.000001):
		for l in range(1,self.length+1):
			...
			self.parameters["w"+str(l)]= self.parameters["w"+str(l)] - learning_rate*self.changes_adam_m["dw" + str(l)]/(1 - beta1**l)/((changes_adam_v["dw" + str(l)]/(1 - beta2**l) + epsilon)**(1/2))
			self.parameters["b"+str(l)]= self.parameters["b"+str(l)] - learning_rate*self.changes_adam_m["db" + str(l)]/(1 - beta1**l)/((changes_adam_v["db" + str(l)]/(1 - beta2**l) + epsilon)**(1/2))
		...
		try:
			...
			for i in range(iterations):
				...
				for batch in batches:
					...
					self.forward_propagation(batch_data)
					self.back_propagation(batch_ylabels)
					self.update_weights_adam(learning_rate = lr, beta1 = beta1, beta2 = beta2)
